...
func (t *TXManager) advanceProgress(tx *Transaction, success bool) error {
	...
	for _, tryEntity := range tx.Components {
		if componentID == "" {
			componentID = tryEntity.ComponentID
		}
		components, err := t.registryCenter.Components(t.ctx, tryEntity.ComponentID)
		...
		if !resp.ACK {
			return fmt.Errorf("tryEntity: %s ack failed", tryEntity.ComponentID)
		}
	}
...
func (t *TXManager) twoPhaseCommit(ctx context.Context, txID string, componentEntities ComponentEntities) (bool, error) {
	...
	// 执行二阶段
	go func() {
		_ = t.advanceProgress(NewTransaction(txID, componentEntities), successful)
	}()
...
func (t *TXManager) getComponents(ctx context.Context, reqs ...*RequestEntity) (ComponentEntities, error) {
	...
	for _, req := range reqs {
		if _, ok := idToReq[req.ComponentID]; ok {
			return nil, fmt.Errorf("repeat tccComponent: %s", req.ComponentID)
		}
	...
	for _, tccComponent := range components {
		entities = append(entities, &ComponentEntity{
			Request:   idToReq[tccComponent.ID()].Request,
